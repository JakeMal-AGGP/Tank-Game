...
namespace LineDraw
{
    public static class LinePrimatives
    {
        ...
        public static void DrawTrapezoid(SpriteBatch spriteBatch, Vector2 position, int xFactor, int xAddition, int yFactor, Color color)
        {
            int xFac = xFactor;
            int xAdd = xAddition;
            int yFac = yFactor;

            Vector2 A = new Vector2(position.X - xFac, position.Y - yFac);
            Vector2 B = new Vector2(position.X + xFac, position.Y - yFac);
            Vector2 C = new Vector2(position.X - xFac - xAdd, position.Y + yFac);
            Vector2 D = new Vector2(position.X + xFac + xAdd, position.Y + yFac);

            LineDrawer.DrawLine(spriteBatch, 3, color, A, B);
            LineDrawer.DrawLine(spriteBatch, 3, color, B, D);
            LineDrawer.DrawLine(spriteBatch, 3, color, D, C);
            LineDrawer.DrawLine(spriteBatch, 3, color, C, A);
        }
